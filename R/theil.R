theil<-function (x=NULL, y=NULL, alpha=0.05, beta.0=0, type="t", example=F, r=3, slopes=F, doplot=TRUE) 
{
  ...
  if(continue)
  {
    ...
    # C from (9.4), S.ij for (9.18)
    CC <- 0
    S.ij <- slopes.table <- numeric(0)
    for(i in 1:(n - 1)) for(j in (i + 1):n)
    {
      if(D.i[j] < D.i[i]) CC <- CC - 1
      if(D.i[j] > D.i[i]) CC <- CC + 1
      S.ij <- c(S.ij, (y[j] - y[i]) / (x[j] - x[i]))
      # Make a table of (i, j, S.ij) for possible printing
    ...
    # C.bar from (9.7), (9.9) or (9.11)
    C.bar <- CC / choose(n, 2)
    
    # Get P-value.  Uses the distribution of Kendall's tau.
    # Needs package SuppDists
    if(type == "t") 
    {
      p <- 2 * pKendall(C.bar, N=n, lower.tail=T)
    ...
    cat("\n")
    cat(paste("C = ", round(CC, r), ", C.bar = ", round(C.bar, r),
              ", P = ", round(p, r), sep=""))
    cat("\n")
    ...
    # Here's a good place to print slopes
    if(slopes)
    {
      cat("\n")
      cat("All slopes:")
    ...
    # Confidence intervals. (9.21) - (9.24) and comment 9
    if(type=="t")	
    {
      a <- alpha / 2
    ...
    # To be conservative, make sure C.alpha gives a lower tail
    # no BIGGER than a (alpha / 2 or alpha):
    if(pKendall(C.alpha, n) > a) 
      C.alpha <- C.alpha - 2 / choose(n, 2)
    # qKendall is for C.bar, so multiple by n(n-1)/2 and use
